...
fn main() {
  ...
  let rows: Vec<Row> = lines
    ...
    .filter_map(|line| {
      if let Some(captures) = ROW_REGEX.captures(&line) {
        let minute = captures["minute"].parse::<i32>().expect("Couldn't parse minute.");
        let guard_id = captures.name("guard_id").and_then(|id| {
          match id.as_str().parse::<i32>() {
            Ok(id) => Some(id),
            Err(_) => None,
          }
        });
        let action = Action::parse_str_to_action(&captures["action"]).expect("Couldn't parse action.");
        return Some(Row { minute: minute, guard_id: guard_id, action: action });
      }
      None
    })
    .collect();
  ...
  let sleepy_guard: (i32, HashMap<i32, i32>) = guard_sleep_map
    .into_iter()
    .max_by(|(_, a), (_, b)| a.values().sum::<i32>().cmp(&b.values().sum::<i32>()))
